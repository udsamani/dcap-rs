...
fn verify_quote_signatures(
    ...
    let mut key = [0u8; 65];
    key[0] = 4;
    key[1..].copy_from_slice(&quote.support.attestation_pub_key);
...
fn verify_tcb_status(
    ...
    if pck_extension.fmspc != tcb_info.fmspc {
        return Err(
            anyhow::anyhow!("tcb fmspc mismatch (pck extension: {:?}, tcb_info: {:?})", pck_extension.fmspc, tcb_info.fmspc)
        )
    }

    if pck_extension.pceid != tcb_info.pce_id {
        return Err(
            anyhow::anyhow!("tcb pceid mismatch (pck extension: {:?}, tcb_info: {:?})", pck_extension.pceid, tcb_info.pce_id)
        )
    }
